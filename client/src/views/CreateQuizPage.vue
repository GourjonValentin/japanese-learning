        ...
        <div class="questions" v-else>
            <form @submit.prevent="submitQuestions">
                <div class="question" v-for="question in quizQuestions" :key="question.id">
                    ...
                    <button @click.prevent="addAnswer(question.id)" type="button" v-if="quizQuestions[question.id].answers.length <= 3">Add anwser choice</button>

                    <button type="button" @click="deleteQuestion(question.id)" v-if="quizQuestions.length > 1">Delete question {{ question.id }}</button>
                </div>
