<template>
    ...
    <div v-else class="edit-div">
        ...
        <div
            ...
            id="mainAttributesForm"
            v-if="isMainAttributesForm"
        >
            <div class="dialog">
                <div class="dialog-header">
                    <h2>Editing Form</h2>
                        ...
                        <div>
                            ...
                            <div class="styledSelectInput">
                                <select
                                    ...
                                    id="quizDifficulty"
                                    v-model="
                                        editedMainAttributes.difficultylevel
                                    "
                                    required
                                >
                                    <option>1</option>
                        ...
                        <div>
                            ...
                            <div class="styledSelectInput">
                                <select
                                    id="quizType"
                                    name="quizType"
                                    v-model="editedMainAttributes.type"
                                >
            ...
            <div class="dialog">
                ...
                <div class="dialog-body">
                    <form @submit.prevent="saveQuestion">
                        ...
                        />
                        <label for="picture" v-if="this.quiz.type === 'anime'"
                            >Picture link :
                        </label>
                        <input
                            class="styledInput"
                            v-if="this.quiz.type === 'anime'"
                            type="text"
                            v-model="editedQuestion.picture"
                            required
                        />
                        <p>Answers :</p>
                        <div
                            class="answers"
                            v-for="answer in editedQuestion.answers || []"
                            :key="answer.id"
                        >
                            <input
                                type="checkbox"
                                :checked="
                                    this.editedQuestion.correct_answers.includes(
                                        answer.id
                                    )
                                "
                                @change="
                                    checkboxChangeHandler($event, answer.id)
                                "
                            />
                            <input
                                class="styledInput"
                        ...
                        <button
                            ...
                            type="button"
                            v-if="editedQuestion.answers.length <= 3"
                        >
                            Add answer choice
                        </button>
        ...
        <div
            class="dialog-overlay"
            id="addingQuestionForm"
            v-if="isAddingQuestionForm"
        >
            <div class="dialog">
                <div class="dialog-header">
                    <!-- Sure que ce soit la bonne indexation ???-->
        ...
        <div class="quiz-content">
            <div
                ...
                v-for="question in quiz.content"
                :key="question.id"
            >
                ...
                <div class="question-header">
                    ...
                    <p v-if="quiz.type === 'anime'">
                        Picture link : {{ question.picture }}
                    </p>
                </div>
                <div
                    class="answer"
                ...
                >
                    <div class="answer-correct-div">
                        <img
                            v-if="question.correct_answers.includes(answer.id)"
                    ...
                    <div
                        :class="{
                            correct: question.correct_answers.includes(
                                answer.id
                            ),
                            answercontent: true,
                        }"
...
export default {
    ...
    methods: {
        async isOwner(quizId) {
            try {
                let res = await axios.get(
                    "http://localhost:3000/quizzes/is-owner",
                    {
                ...
                if (res.status === 200 || res.status === 204) {
                    ...
                    console.error(
                        "Error accessing editing quiz mode for " +
                            this.quiz.id +
                            "\nStatus code: " +
                            res.status
                    );
        ...
        editMainAttributes() {
            this.editedMainAttributes = {
                type: this.quiz.type,
                difficultylevel: this.quiz.difficultylevel,
            };
            this.isMainAttributesForm = true;
        },
        saveMainAttributes() {
            this.quiz.type = this.editedMainAttributes.type;
            this.quiz.difficultylevel =
                this.editedMainAttributes.difficultylevel;
        ...
        deleteQuestionQuiz(question) {
            ...
            this.confirmation.body =
                "You are about to delete this question. Are you sure you want to continue ?";
            const unwatchConfirmation = this.$watch(
                () => this.confirmation.result,
                (newVal) => {
                    if (newVal === true) {
                        this.quiz.content = this.quiz.content.filter(
        ...
        deleteAnswer(answerId) {
            this.editedQuestion.answers = this.editedQuestion.answers.filter(
                (elt) => elt.id !== answerId
            );
            this.editedQuestion.correct_answers =
                this.editedQuestion.correct_answers.filter(
                    (elt) => elt != answerId
                );
        },
        addAnswer() {
            if (this.editedQuestion.answers.length <= 3) {
                this.editedQuestion.answers.push({
                    id:
                        this.editedQuestion.answers[
                            this.editedQuestion.answers.length - 1
                        ].id + 1,
                    content: "",
                });
            }
        ...
        saveQuestion() {
            if (this.editedQuestion.correct_answers.length === 0) {
                ...
                let index = this.quiz.content.findIndex(
                    (elt) => elt.id === this.editedQuestion.id
                );
        ...
        async saveQuiz() {
            if (this.quiz.type === "anime") {
                for (let i in this.quiz.content) {
                    if (this.quiz.content[i].picture === "") {
                        this.alert.title =
                            "Please complete all required field before submission";
            ...
            if (this.quiz.content.length === 0) {
                ...
                this.alert.body =
                    "This quiz contains no questions. Please create at least one question for this quiz.";
                this.isAlert = true;
                return;
            }
            try {
                ...
                if (err.response.status === 404) {
                    this.alert.body =
                        "Oops... The quiz you try to edit does not exist... ";
